...
ブラウザウィンドウを開きます。ウィンドウ上部には **アドレスバー** があり、
現在の URL を表示します。CSS 引数はこのモードでは無視され、
...
| 操作 | 動作 |
|------|------|
| アドレスバーをクリック | アドレスバーを編集状態にする (枠が青くなる) |
| 文字入力 → Enter | 入力した URL/パスへ遷移 (絶対 URL はそのまま、相対は現在ページ基準で解決) |
| 編集中に Esc | 編集を取り消し、現在 URL に戻す |
| `<a href>` を左クリック | そのリンクへ遷移 (相対 href は現在ページ基準で解決) |
| Backspace / ブラウザ Back キー | 履歴を 1 つ戻る (非編集時) |
| マウスホイール | ページを縦スクロール |
| Esc / 閉じるボタン | 終了 (非編集時) |

> 表示バックエンド: WSLg などで `DISPLAY` が到達不能な X サーバを指していると
> X11 で `Broken pipe` になります。本シェルは Wayland ソケットを検出すると
> 自動的に Wayland を選択します (必要なら `WINIT_UNIX_BACKEND=wayland` / `DISPLAY=:0` で上書き)。
