...
import { CROUCH_SPEED, GRAVITY, JUMP_SPEED, PLAYER_HEIGHT, PLAYER_WIDTH, WALK_SPEED } from '../constants'
...
const isSolid = (world: World, x: number, y: number, z: number): boolean => world.getBlock(x, y, z) !== 0
...
export class PlayerController {
  ...
  public update(input: InputController, world: World, dt: number): void {
    ...
    const hasInput = moveForward !== 0 || moveStrafe !== 0
    this.state.velocity[0] = (wishX / wishLength) * speed * (hasInput ? 1 : 0)
    this.state.velocity[2] = (wishZ / wishLength) * speed * (hasInput ? 1 : 0)
  ...
  public getEyePosition(): [number, number, number] {
    return [this.state.position[0], this.state.position[1] + 1.62, this.state.position[2]]
  }
