import type { BlockId, ChunkCoord, ModifiedBlock } from '../types'
...
export class World {
  ...
  public getChunkCoordsByWorld(x: number, z: number): ChunkCoord {
    return { cx: floorDiv(x, CHUNK_SIZE_X), cz: floorDiv(z, CHUNK_SIZE_Z) }
  }
  ...
  public peekChunk(cx: number, cz: number): Chunk | undefined {
    return this.chunks.get(keyOfChunk(cx, cz))
  }
