...
function CircleProgress({ data, index }: CircleProgressProps) {
  ...
  const safeValue = Math.max(0, Math.min(100, data.value));
  const progress = ((100 - safeValue) / 100) * circumference;
    ...
    >
      <div className="relative">
        <svg
          ...
          aria-label={`${data.label} Activity Progress - ${safeValue}%`}
        >
          <title>{`${data.label} Activity Progress - ${safeValue}%`}</title>
...
function DetailedActivityInfo({ activities }: { activities: ActivityData[] }) {
  return (
    <motion.div
      className="flex flex-col gap-6 md:ml-8"
